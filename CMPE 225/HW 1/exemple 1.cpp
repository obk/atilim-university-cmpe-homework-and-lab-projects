	...
	for(int i=0;i<maxbook_ct;i++)
		{
		 ...
		 cout<<"ISBN with"<<isbn<<"does not exsist"<<endl<<endl;


 	}

Library& operator+=(const Book& book){ //ad book with +=
...
ostream &write (std::ostream&dest)const  {
    for(int i=0;i<this->maxbook_ct;i++)
        ...
        cout<<"Borrowed: "<<this->integer[i]<<endl<<"======================================================"<<endl;
...
std::ostream &operator<<(std::ostream &os, Library const &w) { //member ostream overload from stackoverflow
     return w.write(os);
}
...
int main(){
	...
	mazidi.name="Mazidi";elmasri.name="Elmasri";nilsson.name="Nilsson";
...
obj.borrow_book("7899-456456123"); obj.borrow_book("7899-456456123");//barrow book two times
